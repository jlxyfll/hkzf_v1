...
m.getParameters = function (paras) {
    ...
    if (paras.limit && paras.limit.length > 1) {
        index++;
        _self.limit = " limit $" + index + " offset $" + (index + 1);
        ...
        if (paras.update[i].hasOwnProperty('case_field')) {
            ...
            for (let cw in item.case_item) {
                ...
                switch (oper) {
                    case 'replace':
                         ...
                         */
                        case_str.push("WHEN $" + (index - 1) + " THEN $" + index);
                        break;
                    case "plus":
                        /**
                         * 值累加
                         */
                        case_str.push("WHEN $" + (index - 1) + " THEN " + item.key + " + $" + index + " ");
                        break;
                    case "reduce":
                        /**
                         * 值累减
            ...
            switch (oper) {
                case 'replace':
                     ...
                     * 值替换
                     */
                    _self.set.push(paras.update[i].key + '=$' + index);
        ...
        if (paras.keyword[i] != '') {
            ...
            if (paras.keyword[i].hasOwnProperty('operator')) {
                ...
                switch (oper) {
                    ...
                    case ">=":
                    case "<=":
                        _self.where.push(' ' + paras.keyword[i].logic + ' ' + paras.keyword[i].key + oper + "$" + index);
                        //参数化
                        _self.parameters.push(paras.keyword[i].value);
                        break;
                    case '%':
